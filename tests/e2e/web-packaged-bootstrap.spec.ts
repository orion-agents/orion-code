...
test('E2E-P0-01 packaged Host renders v0.3.1 and persists a renamed session', async ({
  artifactState,
  evidence,
  host,
  page,
  ...
  expect(health).toEqual({ status: 200, body: { ok: true, version: '0.3.1' } });

  const bootstrap = await page.evaluate(async () => {
    const response = await fetch('/api/v1/bootstrap', { cache: 'no-store' });
    return response.json();
  });
  expect(bootstrap).toMatchObject({ apiVersion: 1, productVersion: '0.3.1', configured: true });
  expect(bootstrap.nonce).toEqual(expect.any(String));
  expect(JSON.stringify(bootstrap)).not.toContain('orion-web-e2e-test-only');

  await expect(workbenchUi(page).workspaceRail.getByText('v0.3.1', { exact: true })).toBeVisible();
