    ...
    findings: [
      {
        ...
        evidence: 'mock evidence for e2e test',
      },
    ],
    files: [],
    commands: [],
...
jest.mock('../src/runtime/subagents/production', () => ({
  ...
  createChildLlmConfig: jest.requireActual('../src/runtime/subagents/production')
...
describe('subagent end-to-end', () => {
  ...
  it('root LLM invokes subtask and receives structured batch result', async () => {
    ...
    harness.updateContractFromUserInput(
      'parallel research of runtime, session, and tui modules for cancel-semantic regressions'
    );
  ...
  it('preserves subagent usage when root usage is missing before a provider failure', async () => {
    const config = loadConfig({
      apiKey: 'test-key',
      model: 'test-model',
      subagents: { ...DEFAULT_SUBAGENT_CONFIG, mode: 'explicit' },
    });
    const store = new Store({ config, tools: TOOLS, currentModel: 'test-model' });
    const llm = {
      getModel: jest.fn(() => 'test-model'),
      chatStream: jest
              ...
              function: {
                ...
                arguments: JSON.stringify({
                  tasks: [
                    {
                      role: 'research',
                      objective: 'Inspect the failed root turn',
                      reason: 'capture known child usage before root failure',
                    },
                  ],
                  execution: 'serial',
                }),
        ...
        .mockRejectedValueOnce(new Error('root provider failed')),
    };
    const runtime = createRuntime({
      cwd: projectDir,
      config,
    ...
    const evLog = makeEvents();
    const controller = new AgentChatController(runtime, evLog.events);

    await expect(
      controller.runInput('research with a subagent then answer')
    ).resolves.toBeUndefined();
  ...
  it('folds failed child lower-bound usage and marks root accounting incomplete', async () => {
    mockExecuteQuery = async () => {
      throw new SubtaskExecutionError('child failed after provider usage', {
        modelRequests: 1,
        toolCalls: 0,
        promptTokens: 9,
        completionTokens: 4,
    ...
    const llm = {
      ...
      chatStream: jest.fn(async () => {
                    ...
                    tasks: [
                      {
                        role: 'research',
                        objective:
                          'Inspect child usage failure with lower-bound accounting evidence',
                        reason:
                          'independent child check for the same lower-bound accounting objective',
                      },
                    ],
                    execution: 'serial',
                  }),
        ...
        return {
          content: 'continued after isolated child failure',
          model: 'test-model',
    ...
    await controller.runInput(
      'Use a subagent to inspect child usage failure with lower-bound accounting evidence'
    );

    // Keep the child lifecycle observable so a policy rejection cannot masquerade as usage loss.
    expect(evLog.subtaskEventLog.map(event => event.state)).toContain('failed');

    expect(store.getSnapshot().lastLoopStats).toMatchObject({
      subagentPromptTokens: 9,
      subagentCompletionTokens: 4,
      subagentTotalTokens: 13,
  ...
  it('isolates child failure so root continues from successful children', async () => {
      ...
      if (childCallCount === 1) {
        return {
          content: JSON.stringify({
            ...
            findings: [
              {
                title: 'cancel path has dangling handles',
    ...
    const llm = {
      ...
      chatStream: jest.fn(
        async (
                  ...
                  function: {
                    ...
                    arguments: JSON.stringify({
                      tasks: [
                        ...
                        {
                          role: 'research',
                          objective: 'Investigate the session module cancel paths',
                          reason: 'independent',
                        },
                      ],
                    }),
          ...
          return {
            content: 'root analysis after mixed results',
            model: 'test-model',
    ...
    expect(
      evLog4.subtaskEventLog.filter(e => e.state === 'completed').length
    ).toBeGreaterThanOrEqual(1);
    // At least one child failed.
    expect(evLog4.subtaskEventLog.filter(e => e.state === 'failed').length).toBeGreaterThanOrEqual(
      1
    );
  });
});
