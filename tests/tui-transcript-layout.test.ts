...
describe('tui transcript layout', () => {
  it('renders assistant Markdown semantically without source markers or ANSI', () => {
    ...
    expect(spans.some(span => span.text.includes('const answer') && span.style.background)).toBe(
      true
    );
  ...
  it('renders ordinary quotes and ampersands without leaking Markdown entities', () => {
    const rows = layoutTranscriptEntry(
      {
        role: 'assistant',
        content: '被归类为"状态写入工具" & 继续',
      },
      { width: 80 }
    );
    const rendered = allText(rows);

    expect(rendered).toBe('被归类为"状态写入工具" & 继续');
    expect(rendered).not.toMatch(/&(?:quot|amp);/u);
  });
