...
describe('HarnessEvalReceiptV1', () => {
  test('runs the deterministic corpus and evaluates every v0.2.0 gate', () => {
    ...
    expect(result.candidate.summary.orchestrationMsP95).toBe(0.25);
...
function deterministicRuntime(): { readonly now: () => number } {
  let current = 0;
  return {
    now: () => {
