...
export function renderTranscriptInspectorFrame(
  ...
  const footer =
    ' j/k select  Enter/Space expand  Ctrl+E all  [ export  v editor  / search  q exit';
...
function renderList(
  ...
  for (let row = 0; row < maxRows && start + row < view.entries.length; row += 1) {
    ...
    writeFrameText(
      ...
      top + row,
      0,
      truncateWidth(text, Math.max(1, width - 1)),
      selected
...
function sanitizeInlineIdentifier(value: string): string {
  return sanitizeTerminalText(value, 2).replace(/\s+/gu, ' ').trim();
}
