...
function extractPaths(input: string): string[] {
  const matches = input.match(
    /(?:\.{0,2}\/|~\/|\/)[A-Za-z0-9._~/-]+|[A-Za-z0-9._-]+\/[A-Za-z0-9._/-]+\.[A-Za-z0-9]+/g
  );
...
function extractConstraints(input: string): string[] {
  ...
  const constraintPattern =
    /(必须|要求|确保|保持|不要|不能|不准|避免|只|仅|must|should|required|ensure|keep|avoid|without|do not|don't|never|only)/i;
  return [
...
function extractNonGoals(input: string): string[] {
  const lines = input
    ...
    .map(line => line.trim())
    .filter(Boolean);
  const nonGoalPattern =
    /(不要|不能|不准|避免|不引入|不新增|禁止|do not|don't|never|avoid|without)/i;
  return [
...
function isGenericContinue(input: string): boolean {
  return /^(继续|继续吧|继续开发|继续做|接着|接着做|go on|continue|keep going)$/i.test(
    input.trim()
  );
}
...
function isShortFeedback(input: string): boolean {
  ...
  return /^(好的|可以|收到|嗯|对|是的|不错|挺好|挺好的|不对|不对吧|不是|错了|ok|yes|no|fine|great)$/i.test(
    text
  );
}

function isVersionScopedRefinement(input: string): boolean {
  return /(这个版本|本版本|当前版本|this version).*(主要|重点|聚焦|稳定|收敛|优化|完善|focus|stabilize|improve)/i.test(
    input
  );
}
...
function classifyKind(
  input: string,
  state?: HarnessState
): { kind: IntentKind; confidence: number; reason: string } {
  const text = normalize(input);
  ...
  if (
    matches(lower, /\b(verify|test|run tests?|check|lint|typecheck|build)\b/) ||
  ...
  if (
    matches(
      lower,
      /\b(harness|mcp|session|resume|compact|config|configuration|ui|npm|publish|push|pull request|pr)\b/
    ) ||
    matches(text, /(配置|会话|恢复|压缩|版本|分支|发布|新ui|界面|工具|权限|确认|配置文件)/)
  ) {
    if (
      matches(text, /(基于当前|切\s*v|切到|这个版本|新版本|开始按|完整开发|实现这个方案)/) ||
  ...
  if (
    matches(text, /(不对|不是|错了|改成|改为|换成|重新|重做|不要.+而是|应该.+而不是)/) ||
  ...
  if (
    matches(
      text,
      /(基于|切到|这个版本|新版本|开始|实现|开发|生成|创建|写文档|完成本次|按这个计划|完整支持|帮我完成)/
    ) ||
    matches(
      lower,
      /\b(generate|create|implement|build|start|complete this|write a|please implement)\b/
    )
  ) {
    return {
      kind: 'new_task',
  ...
  if (
    matches(
      text,
      /(什么意思|解释一下|解释说明|clarify|explain to me|what does.*mean|what is.*for|can you explain|meaning of)/
    ) ||
  ...
  if (
    matches(
      text,
      /(补充|另外|同时|还有|希望|可以|顺便|完善|优化|灰色|填充|这个|这里|如图|也|继续)/
    ) ||
