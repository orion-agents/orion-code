...
- Collection reads use bounded cursor pagination. Page cursors are opaque and distinct from the
  monotonically increasing numeric SSE cursor.
...
workspace and never exposes the artifact path, but it cannot claim or prove that an artifact
belongs to a particular session. Session-scoped routes require a future provenance-bearing artifact
format and are not part of the v0.3.0 contract.
