...
> 状态：Proposed，完成代码审计，等待拆分 issue 后实施
> 分支：`v0.1.9`（本地）
...
- `package.json` 当前版本仍为 `0.1.8`；只有进入 RC 后才能改版本。
...
当前调研属于“面向架构决策的针对性对标”，不是完整的行业竞品研究。已核对 Claude Code 官方文档和
OpenAI Codex 公开源码；只使用可引用的官方资料、公开实现和可复现实验，不推断闭源内部细节。
...
Phase 0 仍需补齐 Gemini CLI、OpenCode、Aider、Cline 等公开机制，形成
`docs/architecture/compact-benchmark.md`。闭源或无法稳定自动化的能力可标为 `not observable`，不得用猜测补值，
