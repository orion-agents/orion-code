...
> 状态：Superseded，正文保留为历史产品输入，不再作为 v0.2.0 实施主计划
> 版本：v0.2.0
> 调研快照：2026-08-15
> 规划判定：已由 2026-08-23 版本决策取代
> 目标读者：产品、架构、开发、测试、发布负责人
>
> 当前主计划：[v0.2.0 DSH-inspired Harness Architecture Redesign](v0.2.0-dsh-harness-redesign-plan.md)

> [!IMPORTANT]
> v0.2.0 已调整为一次 DSH-inspired Harness 架构大改版：参考 DSH 的能力组织、scope、effect、
> lazy Skill 和轻量 Loop 思想，重构 Orion 自己的 Runtime；不建设第三方插件平台。本文原有的
> Project Mission、isolated worktree、Mission Control、Zed/VS Code 同期交付范围移到 v0.3.0+
> 重新规划。为保留既有调研和用户修改，以下正文不删除、不重写，也不能再作为 v0.2.0 P0 执行依据。
...
2026-08-15 的产品决策进一步把 **Zed ACP、第一方 VS Code 扩展和真实渠道发布**纳入
v0.2.0 Stable。这不是把 IDE 塞进 beta 注脚，而是新增一个有独立预算、兼容契约和发布
Gate 的 Editor Bridge 支柱。为避免再次形成“平台全家桶”，本次 scope swap 同时移出
v0.2.0 的 App Server/TypeScript SDK beta 排期，并继续排除其他 IDE、Desktop、Web 和远程
多客户端。
...
| 终端是唯一产品入口      | TUI、exec、Zed、VS Code 共享同一目标、权限和恢复语义      |
...
2. **Signature leap**：Project Goal、Task Graph、隔离 Writer、Mission Control、Evidence Gate
   形成 Orion 自己的完整产品主循环。
3. **Release proof**：用执行型任务、故障注入、安全对抗、Registry/Marketplace 真实安装证明
   上述能力，而不是用 Demo、测试数量、VSIX 文件或功能表自证。
...
本计划基于 2026-08-15 的本地源码检查：

- 当前分支为 main，HEAD 与 origin/main 均为 7ef3df1，annotated v0.1.8 tag 指向该提交；
- 源码 package 版本为 0.1.8，但 npm registry 仍是 latest=0.1.4、next=0.1.7，且 GitHub
  v0.1.8 Release 不存在；v0.1.8 目前只是 v0.2.0 的候选源码/数据基线；
- 工作树只有既有的 docs/plan/quality-review-rotation-next-run.md 修改，本次规划不接管该改动；
- v0.2.0 开发必须从明确的 post-v0.1.8 基线创建独立分支和发布边界；
- 本文只更新规划和项目目标，不代表 ACP、扩展或外部渠道发布已经实现。
...
| TUI                | v0.1.8 typed TUI、queue、主题、PTY 回归                            | 缺目标总览、任务树、diff/approval/evidence review center             |
...
| Editor bridge      | 有 ACP/Zed 调研文档；AgentRuntimeEvent 可作为适配缝                 | 无 ACP 源码/测试/SDK 依赖；无 VS Code 扩展、manifest 或发布流水线     |
...
| Release            | v0.1.8 source/tag、Jest、PTY、tarball、Node 矩阵基础较强           | v0.1.8 外部发布未闭合；无 ACP Registry / Marketplace 真实发布证据    |
...
- “完成”必须意味着验收条件真的成立；
- 同一套能力可以在 TUI、CI、Zed 和 VS Code 中使用；
- 编辑器只承载交互和工作区上下文，不接管 Provider 凭据、policy 或 Agent runtime。
...
11. `orion acp` 通过 ACP v1 contract，并可从 ACP Registry 在 Zed 中安装和完成真实旅程；
12. 第一方 VS Code 扩展可从 Marketplace 干净安装，并完成 permission、cancel、reload/resume；
13. TUI、exec、Zed 和 VS Code 对重叠 Goal、Task、permission、artifact、evidence 语义一致。

## 3. 范围锁定

### 3.1 v0.2.0 Stable P0：七个发布支柱
...
| P0-G Editor Bridge           | stable ACP v1、Zed ACP Registry、第一方 VS Code 扩展、Marketplace/VSIX、跨客户端一致性       |
...
本地 App Server 和真实 TypeScript SDK 移至 v0.2.x backlog，不再占用 v0.2.0 工期。所有 beta
client 仍必须调用同一 Application API、共用 policy 和 event schema；没有端到端实现的 SDK
mock 必须从公开 stable exports 移除。
...
- 完整 Hooks 生命周期、Orion 第三方 Plugin manifest 和第三方插件市场；
...
- Zed 与 VS Code 之外的 IDE、Desktop、Web、远程 workspace、云同步、团队控制台；
- Agent 自动 push、PR、publish、deploy 等高影响外部工作流。

这里的“插件市场”不包括本版本明确承诺的第一方 VS Code Marketplace 分发；“自动 publish”
也不包括由发布负责人显式授权、带 artifact receipt 的 v0.2.0 发布工程。
...
- 2026-08-15 已批准一次 scope rebaseline：新增 P0-G，移出 App Server/SDK beta，并把总工期
  从 28～36 pw 重估为 41～53 pw；
- 此后新需求必须替换同等工作量的既有 P0，不能只追加；
...
flowchart TB
  ...
  EXEC["orion exec"] --> APP
  ZED["Zed ACP Client"] --> ACP["orion acp / ACP v1"]
  VSCODE["VS Code Extension / ACP Client"] --> ACP
  ACP --> APP
...
1. TUI、exec、ACP 和第一方 editor client 只能通过 Application API 改变状态；
...
6. ACP adapter 只做协议翻译；VS Code 扩展不得 import CLI 私有 runtime 或 mock SDK；
7. 不再维护第二套正式 Agent runtime、UI runtime 或 mock SDK runtime。
...
### P0-G：Stable Editor Bridge 与分发

ACP 交付：

- 新增 `orion acp`，使用官方 `@agentclientprotocol/sdk` 的 stable v1，不手写平行
  JSON-RPC schema，不使用 experimental v2；
- stdio 使用 UTF-8 newline-delimited frames；stdout 只允许协议帧，脱敏日志只写 stderr；
- Orion 主包继续 CommonJS；ACP 使用独立 NodeNext/ESM entry 与 build，避免全仓 ESM 迁移；
- 实现 initialize/authenticate、session new/list/load/prompt/cancel、config options、typed
  session update、tool call/diff、usage 和 permission；未实现能力不得宣告；
- 每条连接拥有独立 controller、abort、event sink、permission correlation 和 MCP scope；
- Project Goal/Task Graph 映射为 ACP plan；标准协议不能表达的字段只能放在 versioned
  `_meta.orion`，并且不能改变 ACP 标准语义；
- policy 先做唯一一次内部判定；确需用户决定时，再发 ACP permission request，禁止双重审批；
- `session/load` 可重放显示状态，但绝不重放 tool 或外部副作用；
- v0.2.0 stable 只承诺 ACP v1 over stdio；Streamable HTTP、WebSocket 和 v2 均不承诺。

Zed 交付：

- 使用 ACP Registry 作为正式安装路径，不创建已弃用的 Zed Agent Server Extension；
- Registry manifest 固定精确 npm 版本，以 `@orion-agents/orion-code@<exact>` + `acp`
  启动，不使用 `latest`；
- 提供符合 Registry 规范的 icon、metadata 和 terminal/agent auth method；
- 在从 Registry 安装的 Zed 中验证 setup/auth、new/prompt、plan、typed tool/diff、permission
  allow/reject、cancel、restart、list/load 和 thread isolation；
- Registry PR、PR merge、CDN entry 和 Zed 真机安装分别留证，不能互相代替。

VS Code 交付：

- 在 `extensions/vscode/` 建立独立、薄的 Node workspace extension；它作为 ACP client spawn
  兼容版本的 `orion acp`，不得加载 `better-sqlite3` 等 native runtime 到 Extension Host；
- v0.2.0 支持 VS Code Desktop 的本地可信 workspace；Remote SSH、WSL、Dev Container、
  virtual workspace 和 vscode.dev 进入 v0.2.x；
- Activity Bar 提供 session/plan/tool/evidence 状态，复用 VS Code 原生 diff editor、命令、
  status bar 和 OutputChannel；完整 Orion 专有 Mission Control 深度视图可在 v0.2.x 增强；
- 命令至少包含 New/Resume Session、Send Selection、Review Changes、Cancel、Show Logs、Doctor；
- 当前文件、selection、diagnostic 和未保存 buffer 作为带 URI/range/version 的显式附件；
- Restricted Mode 只显示 onboarding/status，不启动 Agent、不运行 tool、不读取 workspace
  配置；Workspace Trust 之后仍必须经过 Orion policy；
- Provider 凭据继续由 Orion 管理；扩展不得写入 settings/workspaceState/log/webview state，
  扩展自身必要秘密只能使用 `ExtensionContext.secrets`；
- 扩展只查找 PATH 或用户通过受控选择器确认的 Orion CLI，不执行 workspace 提供的任意
  executable/args，也不静默安装或升级 CLI；
- 不使用 proposed API、inline completion 或第二套 session/permission/Agent runtime。

分发交付：

- M0 在 publisher portal 实际保留 publisher 和唯一 extension ID；建议候选为
  `orion-agents.orion-code`，未保留前不能把它写成已确认事实；
- 使用 `@vscode/vsce` 打包，PNG icon、README、CHANGELOG、LICENSE、SUPPORT、privacy/security
  文档和 `.vscodeignore` 齐全；
- 对 VS Code Stable/最低支持版本及 macOS、Windows、Linux 运行 Extension Host 测试；
- `vsce ls`、secret scan、VSIX 内容审计、clean-profile install 和 exact CLI compatibility
  smoke 全绿；
- 从 immutable tag 和已验证 artifact 发布，流程不得隐式生成 commit/tag；使用 Microsoft
  Entra workload identity 或当时官方支持的最小权限身份，不新增长期全局 PAT；
- 发布后记录 Marketplace URL、extension/version、VSIX SHA256、tag SHA、目标平台、安装与
  rollback receipt；生成 VSIX 不等于已经发布。

验收：

> Given 用户从 Zed ACP Registry 或 VS Code Marketplace 安装 Orion，When 在可信项目中创建
> session、发送 prompt、审阅 tool/diff、允许或拒绝权限、取消并重启恢复，Then 两端都使用
> 同一 `orion acp`、Application API 和 policy，未授权副作用为零且已有 operation 不重放。

> Given VS Code workspace 未受信任，When 用户执行任一 Orion 命令，Then ACP 子进程未启动、
> 文件/命令/网络副作用为零，并显示获得 Trust 与安装兼容 CLI 的明确步骤。
...
- inspectProject；
- initialize/getCapabilities，并记录 client origin 与协议版本；
- create/read/update/pause/resume/cancel Project Goal；
- plan/replan Goal；
- create/list/load/read session；
...
- subscribe RuntimeEvent，包含 correlation、ordering 和 backpressure 契约。

接口先作为 in-process TypeScript contract 使用。TUI、exec 和 ACP adapter 只能做输入、输出、
错误和 capability 的传输适配，不得复制 Goal、policy、tool 或 Agent 业务逻辑。
...
## 7. Stable Editor Client Boundary

### 7.1 ACP Stable Contract

ACP wire version、schema release 和 npm SDK version 是三个独立版本，必须分别锁定并进入
release receipt。P0 只使用官方 stable v1 entry；capability 必须与真实 handler 一一对应。
一条 stdio 连接对应一个 Orion 子进程，project journal lock 防止同项目并发写；v0.2.0 不为
editor client 引入远程 listener、write lease 或常驻 daemon。

兼容矩阵：

| 维度        | v0.2.0 Stable                                      | v0.2.x / 后续                         |
| ----------- | -------------------------------------------------- | ------------------------------------- |
| ACP wire    | stable v1                                          | v2 draft 不承诺                       |
| Transport   | stdio NDJSON                                       | Streamable HTTP 待稳定；WebSocket 不承诺 |
| Session     | new/list/load/prompt/cancel                         | resume/close/delete/additional roots  |
| Content     | text、resource link、typed tool/plan/diff/usage     | image、embedded resource、audio       |
| File/term   | Orion workspace + typed diff；本地 tool execution   | editor filesystem/terminal proxy      |
| MCP         | Orion 既有 stdio MCP                               | HTTP transport 完成后再宣告           |
| Clients     | Zed、第一方 VS Code Desktop local                  | 其他 IDE、remote/web clients          |

### 7.2 Zed ACP Registry

Zed 的 P0 发布对象是 ACP Registry entry，而不是 Zed extension。Registry entry、npm tarball、
terminal auth 和 icon 必须引用相同的精确版本与包元数据。只有 Registry PR 合并、CDN 可见且
Zed 从 Registry 安装的真实旅程通过，才可以说“已发布到 Zed”。外部 review 等待不计 person-week，
但仍是 stable 发布日历风险。

### 7.3 VS Code Extension

VS Code 扩展是独立 Marketplace artifact 和 ACP client。v0.2.0 的 stable 界面限定为标准 ACP
可表达的 session、stream、plan、tool/diff、permission、cancel、usage 与恢复状态；Orion 特有
的 Task Graph/Evidence 深度交互如果需要私有 wire command，必须标为 preview 或推迟，不能
污染 ACP 标准或形成第二套稳定协议。

扩展与 CLI 独立版本化。握手至少校验 extension version、支持的 ACP wire、Orion CLI semver
和 capability；不兼容时 fail closed 并给出精确安装命令，不自动降级或静默换版本。

### 7.4 Distribution Truth

以下状态必须独立记录：source merged、npm published、Registry PR、Registry merged、CDN visible、
VSIX built、Marketplace published、clean install smoke。任何一个本地 artifact 或 CI 绿灯都不能
替代外部渠道状态。Marketplace rollback 通过已验证 patch version 或渠道回退完成，不依赖
unpublish；发布凭据不进入仓库、VSIX、日志或 release receipt。

### 7.5 v0.2.x Backlog

本地 App Server、TypeScript SDK、remote editor host、完整 Mission Control editor protocol、
multi-client lease、HTTP transport 和其他 IDE 进入 v0.2.x；如果后续实现，仍必须复用同一
Application API、policy 和 RuntimeEvent，不得以 ACP/VS Code 为由复制业务核心。
...
| Editor Integration Scenarios |   18 | ACP contract 8、Zed real-client 4、VS Code/Marketplace 6    |
...
| ACP contract               | stable v1 schema/capability/stdout purity fixture 100% conformance      |
| Client parity              | TUI/exec/ACP 重叠 Goal/permission/cancel/evidence 语义 100% 一致       |
| Zed distribution           | Registry merged + CDN visible + Registry clean install 真实旅程通过    |
| VS Code distribution       | Marketplace live + clean profile install + trusted/untrusted 旅程通过  |
| Editor negative paths      | reject/cancel/reload/version mismatch/untrusted workspace 100% 无副作用 |
...
v0.2.0 在加入 Stable Editor Bridge 后是约 41～53 person-weeks 的大版本。单人串行实现约
10～13 个月；3 人小组在依赖清晰、不扩范围且 editor lane 并行的情况下约 18～24 周。
Registry/Marketplace 外部审核时间另计，M0 后再根据真实基线承诺日期。

| Milestone                         |    估算 | 交付                                                               | 退出 Gate                                               |
| --------------------------------- | ------: | ------------------------------------------------------------------ | ------------------------------------------------------- |
| M0 Release Boundary / Baseline    | 2～3 pw | v0.1.8 边界、OAQS、ID/publisher 预留、ADR、范围冻结                 | 独立 v0.2 分支；48+12+24+12+18 manifest 可运行          |
| M1 Application / Event Foundation | 5～6 pw | Application API、RuntimeEventV2、client contract、journal/reducer  | replay/crash；TUI/exec/ACP adapter 无业务分叉           |
...
| M6 ACP v1 / Zed                    | 4～6 pw | ESM ACP adapter、typed events、auth、Registry、Zed                  | ACP 8 场景 + Registry/Zed exact-package Gate            |
| M7 VS Code Extension               | 6～8 pw | thin ACP client、Trust/UI/recovery、VSIX、Marketplace               | VS Code 6 场景 + clean-profile Marketplace Gate         |
| M8 Hardening / Distribution / RC  | 5～6 pw | 性能、迁移、清理、文档、benchmark、全渠道 release receipt          | 全部 Stable Gate + Go/No-Go receipt                     |
...
flowchart LR
  ...
  M1 --> M6["M6 ACP v1 + Zed"]
  M4 --> M6
  M6 --> M7["M7 VS Code Extension"]
  M5 --> M8["M8 Hardening + Distribution + RC"]
  M7 --> M8
```

存在两条必须同时闭合的关键路径：Mission lane 为 M0 → M1 → M2 → M3 → M5 → M8；
Editor lane 为 M0 → M1 → M4 → M6 → M7 → M8。App Server/SDK 不进入 v0.2.0 排期，不能
挤占 Editor lane。

## 10. Epic / Issue 拆分

建议拆成 16 个 Epic、55～80 个窄 Issue。每个 Issue 必须包含允许修改路径、验收命令、证据、
...
| V20-12 ACP v1 Adapter          | V20-03/05/09    | src/acp、acp entry、build             | schema/capability/stdout/session contract |
| V20-13 Zed Registry Delivery   | V20-12          | registry assets、auth、docs           | PR merged、CDN visible、Zed real-client   |
| V20-14 VS Code Extension       | V20-12          | extensions/vscode                     | thin ACP client、Trust、native UX         |
| V20-15 Editor Safety / Parity  | V20-10/11/13/14 | tests/evals/editor                    | 18 场景、crash/cancel/parity 全绿         |
| V20-16 Migration / Dist / RC   | 全部            | docs、scripts、tests、package、CI      | OAQS、npm/Registry/Marketplace receipt    |

App Server 和 TypeScript SDK 转入 v0.2.x backlog，不再作为 v0.2.0 Beta Epic。
...
> Given CI 使用 orion exec --json，When 任务需要人工审批，Then stdout 仍是合法 JSONL、
> exit code=3、stderr 有诊断、workspace 无未授权写入，并可用 session id 恢复。

### Journey 8：迁移与回滚

> Given v0.1.x session 和 active Goal，When 运行迁移 preview、执行并故障注入，Then 旧数据保留、
> 新数据校验失败时不切换，用户可回滚并继续使用原版本。

### Journey 9：Zed ACP Registry

> Given 用户没有本地源码 checkout，When 从 Zed ACP Registry 安装精确版本并完成 terminal
> auth，Then 可 new/prompt、查看 plan/tool/diff、allow/reject permission、cancel，并在重启后
> list/load；ACP logs 无秘密或非协议 stdout。

### Journey 10：VS Code Marketplace

> Given clean VS Code profile、可信 workspace 和兼容的 Orion CLI，When 从 Marketplace 安装
> 扩展并 reload window，Then 可创建/恢复 session、发送 selection、查看原生 diff、拒绝工具、
> cancel 和恢复；deactivate 后无泄漏子进程。

### Journey 11：跨客户端语义一致

> Given 同一 Project Goal、workspace fingerprint 和 policy，When 分别从 TUI、exec、Zed 和
> VS Code 推进等价步骤，Then Goal/Task/permission/artifact/evidence 的状态转换和拒绝理由一致，
> client 不产生第二次 policy decision。

### Journey 12：Editor fail-closed

> Given VS Code workspace untrusted、ACP/CLI 版本不兼容或 editor 子进程崩溃，When 用户发起
> prompt，Then Orion 不执行 tool、不写 workspace、不自动降级版本；错误包含兼容版本、恢复
> 命令和未知 operation 的人工复核入口。
...
- 对齐 v0.1.8 source/tag、缺失的 npm/GitHub Release 状态和当前 dirty tree 所有权；
- 从 7ef3df1 或显式批准的 post-v0.1.8 commit 创建 v0.2 开发分支；
- 保存 v0.1.8 数据 fixture 和 npm tarball 作为迁移输入；
- 在 M0 保留 ACP Registry id、Marketplace publisher/extension id，并记录外部 owner。
...
- CLI、ACP adapter 和 VS Code extension 独立版本化，并维护明确的兼容矩阵；
- VS Code 扩展不得静默安装、升级或切换外部 Orion CLI。

### 12.4 回滚

回滚不得依赖 git reset --hard 或删除用户工作树。版本回滚只切换 Orion 自身 schema pointer，
保留新版本 journal/artifact 供再次迁移或人工导出。Registry/Marketplace 使用已验证 patch
版本或渠道回退，不把 unpublish 当作正常回滚策略。

## 13. 风险与控制

| 风险                              | 预警                             | 控制                                                  |
| --------------------------------- | -------------------------------- | ----------------------------------------------------- |
| 再次范围膨胀                      | 七支柱外再出现远程/生态需求       | Scope swap；新增必须移出同等工作量                    |
...
| ACP stdout / capability 失真      | Zed 解析失败或调用无 handler      | 专用 ESM entry、stdout purity、capability contract     |
| Editor 双重权限或路径逃逸         | client 与 Orion 各决策一次        | policy 单一决策、correlation、canonical path negative  |
| Extension Host / native ABI       | VS Code crash 或平台不一致        | thin client 子进程；Extension Host 不加载 native runtime |
| 子进程泄漏或版本偏移              | reload/cancel 后仍运行             | supervisor teardown、handshake、fail closed            |
| 外部渠道与 npm 状态分裂           | package/Registry/Marketplace 不同版 | exact version、分状态 receipt、渠道 smoke             |
| Publisher credential 风险         | 长期 PAT 或 secret 进入 artifact  | Entra/最小权限身份、secret scan、人工 release approval |
...
- ACP initialize/new/prompt/stdout contract 可用；
- 不要求 UI 完整。

### Beta

- M3/M4/M5 主路径完成，M6 ACP adapter 功能冻结；
- 12 个 Project Mission 至少跑完一轮；
- Zed local/custom agent 与本地 VSIX 跑通真实 permission/cancel/reload 旅程；
- 无已知 critical 安全、数据损坏或 false completion；
- 文档明确 stable 与 beta surface。

### RC

- 48 Atomic × 3、12 Mission × 3、24 safety/recovery、18 editor scenarios 全部执行；
- migration soak、Node/OS、PTY、tarball clean install 完成；
- 精确 npm candidate、Registry CI、Marketplace pre-release/私有验证 artifact 完成；
- VS Code Stable/最低支持版本与 macOS/Windows/Linux Extension Host matrix 完成；
- 所有 P0 Issue 关闭；
...
- ACP capability 超过真实 handler，或 stdout 被日志/banner 污染；
- 只完成本地 Zed custom agent，却声称 ACP Registry 已发布；
- 只生成 VSIX，却声称 Marketplace 已发布；
- Registry PR 未合并/CDN 未可见，或 Marketplace listing 无法 clean install；
- editor client 使用 mock SDK、复制 runtime 或产生第二次 permission decision；
- cancel/reload 后 tool 或子进程继续运行；
- untrusted VS Code workspace 仍启动 Agent、读取配置或产生副作用；
- npm、Registry、VS Code extension 的 compatibility matrix 不一致。

Stable receipt 还必须同时观察到：npm 精确版本可安装、ACP Registry/CDN 已列出且 Zed 可安装、
Marketplace listing 已列出且 clean VS Code 可安装，两端 permission/cancel/load 真实旅程通过。
...
- [ACP Registry contribution contract](https://github.com/agentclientprotocol/registry/blob/main/CONTRIBUTING.md)
- [Zed External Agents](https://zed.dev/docs/ai/external-agents)
- [Zed deprecated Agent Server Extensions](https://zed.dev/docs/extensions/agent-servers)
...
- [Extension continuous integration](https://code.visualstudio.com/api/working-with-extensions/continuous-integration)
...
- [Zed ACP integration research](../zed-acp-integration.md)
- [v0.1.8 plan](v0.1.8-plan.md)
...
按本文七支柱和 41～53 person-weeks 的重估执行：CONDITIONAL GO。若仍按原 28～36 pw、
把 editor publishing 当作附加项，则结论为 NO-GO。重估后的投入集中在一个可演示、
可日用、可评测的完整主循环：

> Project Goal + Task Graph + Isolated Agents + Mission Control + Evidence Gate + Stable Exec
> + Stable Editor Bridge。
